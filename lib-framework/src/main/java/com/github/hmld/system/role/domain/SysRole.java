...
public class SysRole {
	 ...
	 * @param sysRole
	 */
	public static void retryEnity(SysRole sysRole) throws EnityRequiredException{
		if (StringUtils.isNull(sysRole)) {
			throw new EnityRequiredException("数据体不能为空");
		}
		if (StringUtils.isNotEmpty(sysRole.getRolePk())) {
			throw new EnityRequiredException("数据主键不能为空");
		}
		if (StringUtils.isNotEmpty(sysRole.getRoleName())) {
			throw new EnityRequiredException("数据角色名称不能为空");
		}
		if (StringUtils.isNotEmpty(sysRole.getRoleKey())) {
			throw new EnityRequiredException("数据角色键值不能为空");
		}
	}
}