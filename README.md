# 一个普通的 springboot 项目

### 前端项目 web_app
```
前端项目基于 vue-admin-template 和 RuoYi-Vue 版本的前端项目改造而来
vue-admin-template 项目地址：https://github.com/PanJiaChen/vue-admin-template
RuoYi-Vue 项目地址：https://gitee.com/y_project/RuoYi-Vue
```

### 该项目引用到的网络上的代码
```
com.github.hmld.common.utils.MD5Util，来源：
版权声明：本文为CSDN博主「鱼七喵」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/qq_34113011/article/details/107840853
com.github.hmld.common.utils.EncryptUtil，来源：
版权声明：本文为CSDN博主「paavochan」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/theUncle/article/details/100156976
其余部分来自于 RuoYi-Vue 项目的源码学习
RuoYi-Vue 项目地址：https://gitee.com/y_project/RuoYi-Vue
```
### freemaker笔记
```
想要输出#{} 或者 ${} 格式的内容 需要如下写法：
${r"#{"}${value}} 用来生成 #{value} 
${r"${"}${value}} 用来生成 ${value} 
```